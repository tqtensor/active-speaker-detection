...
def compute_average_precision(precision, recall):
    """Computes Average Precision according to the definition in VOCdevkit.

    Modifies precision to ensure that it does not decrease as recall
    decreases.
...
def merge_groundtruth_and_predictions(df_groundtruth, df_predictions):
    ...
    if df_groundtruth["uid"].count() != df_predictions["uid"].count():
        raise ValueError(
            "Groundtruth and predictions CSV must have the same number of unique rows."
        )
    ...
    df_merged["bounding_box_correct"] = np.where(
        eq(
            df_merged["entity_box_x1_groundtruth"],
            df_merged["entity_box_x1_prediction"],
        )
        & eq(
            df_merged["entity_box_x2_groundtruth"],
            df_merged["entity_box_x2_prediction"],
        )
        & eq(
            df_merged["entity_box_y1_groundtruth"],
            df_merged["entity_box_y1_prediction"],
        )
        & eq(
            df_merged["entity_box_y2_groundtruth"],
            df_merged["entity_box_y2_prediction"],
        ),
        True,
        False,
    )
...
def calculate_precision_recall(df_merged):
    ...
    df_merged["is_tp"] = np.where(
        ...
        & (df_merged["label_prediction"] == "SPEAKING_AUDIBLE"),
        1,
        0,
    )
    ...
    logging.info(
        "\n%s\n",
        df_merged.head(10)[
            ["uid", "score", "label_groundtruth", "is_tp", "tp", "precision", "recall"]
        ],
    )
